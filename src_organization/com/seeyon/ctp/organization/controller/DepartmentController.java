...
//		Map postMap = organization.getAllEntity(V3xOrgPost.class, dept.getOrgAccountId());
		...
		String titleManager = "";
		if (null != managerList) {
		    for(V3xOrgMember manager : managerList){
		        if(Strings.isBlank(titleManager)){
		            titleManager = manager.getName();
		        }else{
		            titleManager = titleManager + "," + manager.getName();
		        }
		    }
		}
		
		String showManager = titleManager;
		if(showManager.length()>16){
			showManager = showManager.substring(0,16)+"...";
		}
		if (Strings.isEmpty(titleManager)) {
		    titleManager = null;
		}
		result.addObject("showManager", showManager);
		result.addObject("titleManager", titleManager);
