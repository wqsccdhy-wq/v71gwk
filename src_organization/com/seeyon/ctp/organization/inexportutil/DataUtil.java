...
import com.seeyon.ctp.organization.inexportutil.datatableobj.BusinessDepartmentOpr;
import com.seeyon.ctp.organization.inexportutil.datatableobj.DepartmentOpr;
import com.seeyon.ctp.organization.inexportutil.datatableobj.ExtMemberOpr;
...
//	}
	...
	public static List setResultList(List resultlst,List volst,String repeat,Locale locale ){
		String resource = "com.seeyon.ctp.organization.resources.i18n.OrganizationResources";	
	...
	public IImexPort getInstance(String type){
		if(IPublicPara.member.indexOf(type) != -1){
			return new MemberOpr();
		}else if("extMember".equals(type)){
			return new ExtMemberOpr();
		}else if(IPublicPara.post.indexOf(type) != -1){
			return new PostOpr();
		}else if("businessDepartment".indexOf(type) != -1){
            return new BusinessDepartmentOpr();
        }else if("department".indexOf(type) != -1){
	 ...
	 * @throws Exception
	 */
	public Map devideVo(OrgManager od,List volst) throws Exception{
...
//	}	
	 ...
	 * @throws Exception 
	 */
	public List getMatchValue(OrgManager od,MetadataManager metadataManager, Long accountid,List<List<String>> accountList,List volst ) throws Exception{
...
//	}
	...
	public static final String NULL_CURRENTUSER=ResourceUtil.getString("org.dataUtil.no.current.user");
	public static final String IMP_EXP_DOING=ResourceUtil.getString("org.dataUtil.deal.with.data");
...
//			sheet.setDefaultColumnWidth((short) 12);
			...
			if (colunmLength > 0) {
				sheet.addMergedRegion(new CellRangeAddress(0, (short) 0, 0, (short) (colunmLength - 1)));
			}
			...
			for (int c = 0; c < dataRows.length; c++) { // 多行数据
				...
				row.setHeightInPoints((float) 20);
				if(null==dataRows[c]) continue;//屏蔽错误数据
				short height = dataRows[c].getHeight();
				if(height!=0){
				...
				for (int j = 0; j < dataCells.length; j++) { // 一行数据多单元格
					...
					case DataCell.DATA_TYPE_INTEGER:
						...
						if(dataCell.getContent()==null||"".equals(dataCell.getContent())){
							cell.setCellValue("");
						}else{
