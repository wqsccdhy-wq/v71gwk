...
*/
    ...
    public ModelAndView importReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (null == request.getParameter("page")) {
            //置空信息结果
            session.setAttribute("resultlst",null);
        }
        String selectvalue = request.getParameter("selectvalue");
        if (selectvalue == null || "".equals(selectvalue) || "null".equals(selectvalue)) {
            throw new Exception("请上传文件对应的表！");
        ...
        //判断数据唯一性的标识，这里只给部门导入用。（默认为名称）
        String onlytag = request.getParameter("onlytag") == null ? "name" : request.getParameter("onlytag");
        ...
        modelAndView.addObject("onlytag", onlytag);
        AppContext.putSessionContext("onlytag", onlytag);
...
//		}
        ...
        if (accountList != null && accountList.size() > 2) {
            ...
            if("extMember".equals(selectvalue)){
            	reall = accountList.subList(3, accountList.size());
            }else{
            	reall = accountList.subList(("department".equals(selectvalue) || "businessDepartment".equals(selectvalue))?0:2, accountList.size());
            }
