...
public class AccountManagerImpl implements AccountManager {
    ...
    protected DistributeManager     distributeManager;
	...
	public void setDistributeManager(DistributeManager distributeManager) {
    ...
    public FlipInfo showAccounts(FlipInfo fi, Map params) throws BusinessException {
        ...
        /********************/
        List<V3xOrgAccount> accountBOs = new UniqueList<V3xOrgAccount>();
        List<OrgUnit> units = new UniqueList<OrgUnit>();
        units = orgDao.getAllUnitPO(OrgConstants.UnitType.Account, null, enabled, true, condition, value, null);
        accountBOs = (List<V3xOrgAccount>) OrgHelper.listPoTolistBo(units);
        
        if ((orgManager.isRole(user.getId(), accountId, OrgConstants.Role_NAME.AccountAdministrator.name())
        		|| orgManager.isRole(user.getId(), accountId, OrgConstants.Role_NAME.HrAdmin.name())) && account.allowManagementSubunit()){
    		List<V3xOrgAccount> orgAccounts0 = new UniqueList<V3xOrgAccount>();
    		String currentPath = account.getPath();
    		List<V3xOrgAccount> accessAccount = orgManager.accessableAccounts(user.getId(),true);
    		for(V3xOrgAccount a : accountBOs){
    			if(a.getPath().startsWith(currentPath) && accessAccount.contains(a)){
    				orgAccounts0.add(a);
    			}
    		}
    		accountBOs = orgAccounts0;
        }
        
        Collections.sort(accountBOs, CompareSortEntity.getInstance());
        List<V3xOrgAccount> pageList = DBAgent.memoryPaging(accountBOs, fi);
    ...
    public Long updateAcc(Map map) throws BusinessException {
        ...
        if("on".equals(String.valueOf(map.get("managementSubunit")))) {
            map.put("managementSubunit", 1L);
        } else {
            map.put("managementSubunit", 0L);
        }
        ...
        if (OrgConstants.GROUPID.equals(updateAccount.getId())) {
            ...
            if (null != map.get("checkManager") && "1".equals(map.get("checkManager").toString())) {
            	if(!updateAccount.getId().equals(AppContext.currentAccountId())){
    ...
    @Override
    @CheckRoleAccess(roleTypes={Role_NAME.AccountAdministrator,Role_NAME.GroupAdmin,Role_NAME.HrAdmin,Role_NAME.AccountAdministrator})
    public Long updateAcc4Admin(Map map) throws BusinessException {
        ...
        if(oldEntity.allowManagementSubunit()){
        	updateAccount.setProperty("managementSubunit", Long.valueOf(1));
        }
        
        OrganizationMessage message = null;
        if(AppContext.getCurrentUser().getLoginAccount().equals(updateAccount.getId())){//如果当前人员登录的单位和要修改的单位一致，则不能修改自己所在单位的单位访问范围。
        	List<V3xOrgAccount> list = new ArrayList<V3xOrgAccount>();
        	updateAccount.setProperty("accessLevels", oldEntity.getProperty("accessLevels"));
        	updateAccount.setProperty("permissionType", oldEntity.getProperty("permissionType"));
        	list.add(updateAccount);
        	message = orgManagerDirect.updateAccounts(list,false);
        }else{
        	message = orgManagerDirect.updateAccount(updateAccount);
        }
        OrgHelper.throwBusinessExceptionTools(message);
    ...
    public HashMap viewAccount(Long accountId) throws BusinessException {
        ...
        V3xOrgAccount currentAccount = orgManager.getAccountById(currentAccountId);
        ...
        if(viewAccount.allowManagementSubunit()) {
            map.put("managementSubunit", true);
        } else {
            map.put("managementSubunit", false);
        }
        ...
        if (viewAccount.isGroup()) {
            ...
            map.put("accountSize", orgAccounts.size() - 1);
            ...
            if(isGroupVer && !user.isGroupAdmin() && currentAccount.allowManagementSubunit()){
            	List<V3xOrgAccount> orgAccounts = orgManagerDirect.getAllAccounts(null, true, null, null, null);
            	List<V3xOrgAccount> orgAccounts0 = new UniqueList<V3xOrgAccount>();
            	String currentPath = currentAccount.getPath();
            	...
            	map.put("mAccountSize", orgAccounts0.size());
            	
            	FlipInfo fi1 = conPostManager.list4SubUnitManage(new FlipInfo(), new HashMap());
            	map.put("mConcurrentSize", fi1.getTotal());
            	
            	FlipInfo fi2 = distributeManager.showDistributeList(new FlipInfo(), new HashMap());
            	map.put("mRedistributionSize", fi2.getTotal());
            }
...
//                V3xOrgPrincipal adminPrincipal = adminMember.getV3xOrgPrincipal();
        ...
        boolean hasAccountbindPlugin = false;
        if(AppContext.hasPlugin("accountbind")){
        	hasAccountbindPlugin = true;
        }
        map.put("hasAccountbindPlugin", hasAccountbindPlugin);
    ...
    @Override
    @CheckRoleAccess(roleTypes={Role_NAME.SystemAdmin,Role_NAME.GroupAdmin,Role_NAME.AccountAdministrator,Role_NAME.HrAdmin})
    public List<WebV3xOrgAccount> showAccountTree(Map params) throws BusinessException {
    	...
    	boolean isGroupVer = (Boolean) (SysFlag.sys_isGroupVer.getFlag());
    	boolean managementSubunit = account.allowManagementSubunit();
    	
    	List<V3xOrgAccount> orgAccounts = orgManagerDirect.getAllAccounts(null, true, null, null, null);
    	if(isGroupVer && !user.isGroupAdmin() && managementSubunit){//取自己单位和子单位,并且过滤不可见的单位
    		List<V3xOrgAccount> orgAccounts0 = new UniqueList<V3xOrgAccount>();
    		String currentPath = account.getPath();
    		List<V3xOrgAccount> accessAccount = orgManager.accessableAccounts(user.getId(),true);
    		for(V3xOrgAccount a : orgAccounts){
    		...
    		orgAccounts = orgAccounts0;
    		V3xOrgAccount managerNode = orgManager.getAccountById(parentAccountId);//虚拟的管理子单位的节点
        	managerNode.setName(ResourceUtil.getString("org.manager.rootnode"));
        	orgAccounts.add(managerNode);
    	}
        ...
        for (V3xOrgAccount a : orgAccounts) {
        	...
        	Long pId = -1L;
        	if(a.isGroup() || a.getId().equals(parentAccountId)){
        		pId = -1L;
        	}else{
        		pId = accountId;
        	}
        	Long parentId = accId2parentAcc.get(a.getId())==null?pId:accId2parentAcc.get(a.getId());
	...
	public HashMap getAccountCustomerRoles(Long accountId) {
		...
		try {
			List<V3xOrgRole> disableRoleList = orgManager.getPlugDisableRole(accountId);
	...
	@AjaxAccess
	@Override
	public List<Map<String,Object>> getChildMapList(Long pId) throws BusinessException {
		User user = AppContext.getCurrentUser();
		
		List<Map<String,Object>> result = new ArrayList<Map<String, Object>>();
        
    	List<Map<String,Object>> treeList = new ArrayList<Map<String, Object>>();
  		List<V3xOrgAccount> allChildrenAccount = orgManager.getChildAccount(pId, true);
  		List<V3xOrgAccount> accessAccount = orgManager.accessableAccounts(user.getId());
  		List<V3xOrgAccount> allList = (List<V3xOrgAccount>) CollectionUtils.intersection(allChildrenAccount, accessAccount);
  		
		for(V3xOrgAccount a : allList){
			...
			tempAccount.put("hasChildren",Strings.isEmpty(orgManager.getChildAccount(a.getId(), true)) ? false : true);
			tempAccount.put("level","-1");
			tempAccount.put("num",num);
