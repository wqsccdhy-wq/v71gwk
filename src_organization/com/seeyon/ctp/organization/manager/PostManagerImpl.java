...
public class PostManagerImpl implements PostManager {
    ...
    public Long createPost(String accountId, Map post) throws BusinessException {
        ...
        if (newpost.getId() == null) {
            ...
            //记录日志
            appLogManager.insertLog4Account(user, user.isGroupAdmin() ? accId : AppContext.currentAccountId(), AppLogAction.Organization_NewPost, user.getName(), newpost.getName());
        } else {
            //排序号的重复处理
            String isInsert = post.get("sortIdtype").toString();
            if ("1".equals(isInsert)
            ...
            OrgHelper.throwBusinessExceptionTools(returnMessage);
            //记录日志
            appLogManager.insertLog4Account(user, user.isGroupAdmin() ? accId : AppContext.currentAccountId(), AppLogAction.Organization_UpdatePost, user.getName(), newpost.getName());
        }
    ...
    public void createPostFormBase(String accountId, String posts) throws BusinessException {
        ...
        //记录日志
        appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? accId : AppContext.currentAccountId(), AppLogAction.Organization_NewPost, applogs);
    ...
    private void dealPostName(V3xOrgPost p, HashMap m) {
        ...
        if(OrgConstants.ORGENT_STATUS.DISABLED.ordinal() == p.getStatus()) {
            postName = p.getName() + "("+ ResourceUtil.getString("org.entity.disabled") +")";
        } else if(OrgConstants.ORGENT_STATUS.DELETED.ordinal() == p.getStatus()) {
            postName = p.getName() + "("+ ResourceUtil.getString("org.entity.deleted") +")";
        }
        m.put("name", postName);
    }
	...
	public String deletePost(List<Map<String,Object>> post) throws BusinessException {
        ...
        //记录日志
        appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? postlist.get(0).getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_DeletePost, appLogs);
		return null;
	}
