 ...
 * <p>接口维护规则：本接口用于提供外部应用对组织模型信息的查询支持，对组织模型信息维护管理等接口请不要定义于此处</p>
 * 
...
public interface OrgManager {
     ...
     * @param memberId 被比较人员
     * @return true可见false不可见
     */
     ...
     * 
     * @param typeAndId
     *            用"|"组合的字符串，如：Member|-92874958395或者Department|3461234123458
     * @return 组织模型实体
     * @throws BusinessException
     */
    ...
    /**
     * 按照ID取任意类型对象，读取的顺序依次是：人、部门、岗位、单位、职级、组、角色（<b>只读取有效的，停用的不读取</b>）
     * @param id
     * @return
     * @throws BusinessException
     */
    ...
    /**
     * 把组织类型和id用“|”连接，该方法自动分解，返回对应数据
     * <br>用于解析选人界面部门角色和部门岗位返回实体
     * @param typeAndId
     *            用"|"组合的字符串，如：Department_Post|-92874958395_3461234123458
     ...
     * 
     * @param typeAndIds
     *            先用","，再用"|"组合的字符串，如：Member|-92874958395,Department|3461234123458,Department|5435234764545
     * @return 实体列表
     * @throws BusinessException
     */
     ...
     * @param value 属性值
     * @param accountId 单位id
     * @param isPaginate 是否分页:true分页false不分页
     * @return 实体列表
     * @throws BusinessException
     */
    List<V3xOrgEntity> getEntityList(String entityClassName, String property, Object value, Long accountId,
     ...
     * 
     * @param typeAndIds
     *            先用","，再用"|"组合的字符串，如：Member|-92874958395,Department|3461234123458,Department|5435234764545<br>
	 ...
	 * 
	 * @param accountId 单位id
	 * @param firstLayer 是否只取第一层: true只取第一层子单位  false查询所有子单位
	 * @return 子单位列表
	 * @throws BusinessException
	 */
    ...
    /**
     * 获得当前单位所在树的根单位, 当前只能有一个根单位；如果是独立单位就返回<code>null</code>
     * 
     * @param accountId
     * @return
     * @throws BusinessException
     ...
     * @param memberId 人员
     * @return 兼职信息完整的单位列表(兼职信息完整包括:兼职单位，兼职部门，兼职岗位，兼职职务完整)
     * @throws BusinessException
     */
     ...
     * 返回一个成员能够访问的的单位列表
     * @param memberId 人员id
     * @param includeDisable 是否包含无效单位
     * @return 单位列表
     * @throws BusinessException
     */
    List<V3xOrgAccount> accessableAccounts(Long memberId, boolean includeDisable) throws BusinessException;
     ...
     * 得到这个人在指定单位下的岗位信息
     * 
     * @param accountId 可以为<code>null</code>，表示所有单位
     * @param memberId
     * @return
     * @throws BusinessException
     */
     ...
     * @param parentDepId 父部门id
     * @param firtLayer true只查询一层子部门 false查询所有子部门
     * @param isInteranl 是否为内部部门，false==外单位
     * @return
     * @throws BusinessException
     */
     ...
     * @throws BusinessException
     */
    V3xOrgAccount getAccountByPath(String path) throws BusinessException;
     ...
     * 
     * @param unitId
     *            部门或单位的ID , 如果是部门角色，当unitId为空或者设置的是单位的id，表示可以查出单位下所有的部门角色人员
     * @param roleId
     *            角色的ID
     * @throws BusinessException
     */
    ...
    /**
     * 取部门角色对应的所有人员。
     * （仅包含有效人员）判断人员是否有效标识<code>isValid()</code>方法
     * @param departmentId 部门Id
     * @param roleName 角色名称
     * @return 角色对应的所有人员，部门或角色不存在返回size为0的List。
     */
     ...
     * @throws BusinessException
     */
    List<V3xOrgMember> getMembersByDepartmentRoleOfAll(long departmentId,String roleNameOrId) throws BusinessException;
     ...
     * @param id 对应的ID，如果是Department_Role，这是123123434_231234236，兼容系统预置角色123123434_DeptAdmin
     * @return
     * @throws BusinessException
     */
     ...
     * @param accountId 单位Id，如果为<code>null</code>或<code>OrgEntity.VIRTUAL_ACCOUNT_ID</code>则返回全集团的人员名称。
    ...
    /**
     * 取得所有单位的简称。
     * @return 单位Id-简称Map，包含启用、停用和已删除单位。
     * @throws BusinessException
     */
     ...
     * 获得部门下所有外部成员成员
     * @param departmentId 部门id
     * @param firtLayer true只查询本部门  false同时查询子部门
     * @return
     * @throws BusinessException
     */
     ...
     * 获取单位下所有人员
     * @param accountId
     * @param includeChildAcc true时包所有含子单位，false时仅本单位
     * @return
     * @throws BusinessException
     */
    ...
    /**
     * 根据手机查询人员<br>
     * <em>注意：手机号没有唯一判断，系统可能存在某两个人相同手机号，但接口只返回第一个匹配的结果，如果没有查询到则返回null</em>
     * @param mobile 手机号
     ...
     * 根据手机号码获取人员（内部人员）
     * @param mobile 手机号码
     * @param accountId 单位id,(V3xOrgEntity.VIRTUAL_ACCOUNT_ID:在全集团查找)
     * @return
     * @throws BusinessException
     */
    List<V3xOrgMember> getMemberListByMobile(String mobile, Long accountId) throws BusinessException;
     ...
     * @throws BusinessException
     */
    List<V3xOrgMember> getMemberListWithOuterByMobile(String mobile,Long accountId) throws BusinessException;
    ...
    /**
     * 取得所有指定单位引用的基准岗。
     * @param accountId 单位Id；如果值为<code>OrgEntity.VIRTUAL_ACCOUNT_ID</code>，等同于取全集团所有被引用的基准岗，但不包含未被引用的。
     * 
     * @return 指定单位引用的所有基准岗。
     * @throws BusinessException
     */
     ...
     * 
     * @param memberId 指定的人员
     * @param unitId 可以是单位、部门，可以为<b>null</b>；如果是单位角色，这里给的是部门，暂无此情况
     ...
     * 判断人员是否具备指定岗位。
     * @param memberId 人员Id
     * @param postId 岗位Id，岗位为基准岗时检查基准岗下所有岗位
     * @param postTypes 岗位类型组合：主岗、副岗、兼职。<br/>
     *     不传类型时检查所有岗位类型，只要具备一个岗位类型的岗位就返回true；传多个类型时在指定岗位类型内检查。
     ...
     * @param memberId
     * @param deptIdList 部门Id列表，人员只要在其中任何一个部门下就返回<tt>true</tt>。
     * @param includeChild 是否判断子部门
     * @return 人员在指定部门下返回<tt>true</tt>，否则返回<tt>false</tt>。
     * @throws BusinessException 
     */
     ...
     * @param memberPostTypes {@link OrgConstants.MemberPostType} 岗位类型：主管、副岗、兼职，可以为null，表示所有
     * @param deptIdList
     * @param hasChildDep
     * @return
     * @throws BusinessException
...
//    List<OrgAccount> getAllAccounts() throws BusinessException;
	 ...
	 * @throws BusinessException
	 */
    List<V3xOrgDepartment> getAllDepartmentsWidthDisable(Long accountId) throws BusinessException;
     ...
     * @throws BusinessException
     */
    List<V3xOrgRole> getAllRolesByBond(Long accountId, OrgConstants.ROLE_BOND ... types) throws BusinessException;
    /**
     * 取指定单位的所有部门角色。
     * 
     * @param accountId
     ...
     * <br>注意：这个方法会返回人员的岗位、部门、职务所拥有的角色
     * <pre>
     ...
     * @param unitId 组织id
     *              <pre>
     *              1. 可以为<code>null</code>，表示全系统担任所有“单位角色”和“部门角色”
    ...
    /**
            * 取得部门人员。（包括主/副/兼）
     * @param departmentId 部门Id
     * @param firstLayer 为<CODE>true</CODE>时不包含子部门人员，否则取本部门以及所有子部门的人员。
     * @return 部门下的人员
     * @throws BusinessException
     */
    ...
    /**
              * 取得部门人员，包含无效（停用和删除人员）。（包括主/副/兼）
     * @param departmentId
     * @param firtLayer  是否包含子部门。 true: 不包含子部门，false:包含子部门
     * @return
     * @throws BusinessException
     */
    List<V3xOrgMember> getMembersWithDisableByDepartment(Long departmentId, boolean firtLayer) throws BusinessException;
     ...
     * @param depId
     *            部门Id
     * @return 如果具有部门管理员返回<CODE>true</CODE>，否则返回<CODE>false</CODE>。
     * @throws BusinessException
     */
    ...
    /**
     * 取指定单位的所有人员。只包含能够登录的人员<br>
     * 判断人员是否有效标识<code>isValid()</code>方法
     * 
     * @param accountId
     *            单位Id V3xOrgEntity.VIRTUAL_ACCOUNT_ID表示全系统
     * @return 单位人员列表
     */
    ...
    /**
     * 取得一个大Map<姓名, 人员对象>；key同时包括：姓名、姓名(code)、姓名(登录名)；<br>
     ...
     *      刘强(liuq2)
     * </pre>
     * @param accountId V3xOrgEntity.VIRTUAL_ACCOUNT_ID表示全系统
     * @return
     * @throws BusinessException
     */
    ...
    /**
     * 取具有某个岗位的所有有效人员，包括主岗、副岗和兼职为指定岗位的所有人员<br>
     * （仅包含有效人员）判断人员是否有效标识<code>isValid()</code>方法 
     * 
     * @param postId
     *            岗位Id，如果为基准岗，则包括所有基准岗关联岗位的人员。
     * @return 具有指定岗位的所有人员。如果岗位不存在返回size为0的List。
     * @throws BusinessException
     */
     ...
     * @param objectiveIds 可以为<code>null</code>可以为<code>null</code>，value类型只能是Long/String/List&lt;Long&gt;/List&lt;String&gt;
     * @return
     */
     ...
     * 根据设置的url获取单位ID
     * @param customLoginUrl 单位的登录url(不包括ip地址和端口)
     ...
     * @param departmentId
     * @param levelScope
     * @param 是否只包含本部门。为true时不包含子部门人员，否则取本部门以及所有子部门的人员。
     * @return
     * @throws BusinessException
     */
	...
	*/
     ...
     * @throws BusinessException
     */
	List<V3xOrgAccount> getAllParentAccount(Long accountId) throws BusinessException;
	
	/**
	 * 获取集团职务级别在指定单位下映射的单位职务级别
	 * @param GroupLevelId  集团职务级别id
	 * @param accountId  指定单位的ID
	 * @return
	 * @throws BusinessException
	 */
	V3xOrgLevel getLevelByGroupLevelId(Long GroupLevelId, Long accountId) throws BusinessException;
	
	/**
	 * 是否包含子部门
	 * @param parentDepId
	 * @param includeExternal 包含外部部门
	 * @return
	 * @throws BusinessException
	 */
	boolean hasChildren(Long parentDepId, boolean includeExternal)throws BusinessException;
	 ...
	 * @param departmentId
	 * @param firtLayer
	 * @param includeOuter,是否包含下级的编外部门
	 * @return
	 * @throws BusinessException
	 */
	...
	public List<MemberPost> getMemberPostByDepartment(Long departmentId,boolean firtLayer, MemberPostType type) throws BusinessException;
	 ...
	 * 判断应用是否允许访客访问
	 * @param category 应用标识
	 *     如：ApplicationCategoryEnum.meeting.name()
	 * @param accountId 单位ID
	 * @return
	 */
