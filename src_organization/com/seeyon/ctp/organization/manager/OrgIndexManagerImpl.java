...
public class OrgIndexManagerImpl extends AsynchronousBatchTask<Object> implements OrgIndexManager {
    ...
    public String getRecentDataStr(Long memberId, String customType) throws BusinessException {
        ...
        if(Strings.isNotBlank(recentDatas)) {
            ...
            for(String s : oldData) {
            	Long mId = null;
            	try {
            		mId = Long.valueOf(s.split("[|]")[1]);
				} catch (Exception e) {
					logger.error("最近联系人信息有误:"+s);
					continue;
				}
                V3xOrgMember m = orgManager.getMemberById(mId);
                if(null != m && m.isValid()) {//OA-64293 
                    if (m.isVJoinExternal()) {//过滤V-Join人员
                        continue;
                    }
    ...
    public String getSearchDataStr(String key) throws BusinessException {
        ...
        Long currentLoginAccId = user.getLoginAccount();
    	key = key.toLowerCase();
    	List<V3xOrgMember> tempList = new UniqueList<V3xOrgMember>(orgFastSearchLength);
    	if(user.isInternal()) {
    		...
    		for(V3xOrgMember m : wholeMembers) {
    			...
    			if(name.indexOf(key)>=0 && m.isValid() || m.getPinyin().startsWith(key) || m.getPinyinhead().startsWith(key)){
    				if(OrgHelper.checkLevelScope(user.getId(), m.getId())){
    					if(checkAccountAccess(currentMember,m)){
    						tempList.add(OrgHelper.cloneEntity(m));
    						if(searchData.size()>=orgFastSearchLength) break;
    					}
    				}
    			}
    		}
    	}else if(user.isV5External()) {
            //外部人员访问内部人员
            Collection<V3xOrgMember> canAccessMembers = OuterWorkerAuthUtil.getCanAccessMembers(user.getId(),currentMember.getOrgDepartmentId(), currentMember.getOrgAccountId(), orgManager);
    		for(V3xOrgMember m : canAccessMembers) {
    			String name = m.getName().toLowerCase();
    			if(name.indexOf(key)>=0 && m.isValid() || m.getPinyin().startsWith(key) || m.getPinyinhead().startsWith(key)){
					tempList.add(OrgHelper.cloneEntity(m));
					if(searchData.size()>=orgFastSearchLength) break;
        ...
        this.dealDupNameMembers(tempList);
        for(V3xOrgMember m : tempList) {
        	WebEntity4QuickIndex data = null;
            if(currentLoginAccId.equals(m.getOrgAccountId())) {
                data = new WebEntity4QuickIndex("Member|"+m.getId()+"|"+m.getOrgAccountId()+"|"+m.getName(), m.getName(), null);
            } else {
    ...
    private boolean checkAccountAccess(V3xOrgMember currentMember,V3xOrgMember member) throws BusinessException{
    	//如果 1.被访问人员和当前人员不再一个同一个单位
    	//2.被访问人员在当前人员所在单位没有兼职
    	//3.当前单位的可见单位中，没有被访问人员的所在单位
    	//则，此人员也不可见。
    	if(!currentMember.getOrgAccountId().equals(member.getOrgAccountId())){
    		Map<Long, List<MemberPost>> map = orgManager.getConcurentPostsByMemberId(currentMember.getOrgAccountId(), member.getId());
    		if(map == null || map.size()==0){
    			List<V3xOrgAccount> accounts = orgManager.accessableAccounts(currentMember.getId());
    			for(V3xOrgAccount a : accounts){
    				if(a.getId().equals(member.getOrgAccountId())){
    					return true;
    				}
    			}
    			return false;
	...
	public String getAllMembersWithDisable(Long accountId) throws BusinessException {
		...
		for(V3xOrgMember member : memberList){
			...
			if(member.getState()==2){
				fsp.setText(member.getName()+"("+ResourceUtil.getString("org.metadata.member_state.out")+")");
			}
			if(!member.getEnabled()){
				fsp.setText(member.getName()+"("+ResourceUtil.getString("org.member_form.end.rep")+")");
			}
			list.add(fsp);
		}
	...
	private List<V3xOrgEntity> getSimpleDataList(String className,String key,Long accountId) throws BusinessException {
		...
		Set<Long> accessableAccounts = new HashSet<Long>();
		if(accountId == null){//如果不指定查询的单位，快速查询只能查询当前人员可见单位内的数据，和选择控件保持一致
			List<V3xOrgAccount> accounList = this.orgManager.accessableAccounts(AppContext.currentUserId());
			for(V3xOrgAccount account : accounList){
				accessableAccounts.add(account.getId());
			}
		}else{
			accessableAccounts.add(accountId);
		}
		
		
		//先查有效数据
		List<V3xOrgEntity> datas =  orgManager.getEntityNoRelation(className, "name", key, accountId, false, false, true);
		for(V3xOrgEntity entity : datas){
			if(accessableAccounts.contains(entity.getOrgAccountId())){
				result.add(entity);
			}else if(V3xOrgMember.class.getSimpleName().equals(className)){//如果是人人员，还要查兼职岗数据。
		        OrgConstants.MemberPostType[] postType = new OrgConstants.MemberPostType[]{OrgConstants.MemberPostType.Concurrent};
		        List<V3xOrgRelationship> rels = this.orgCache.getMemberPostRelastionships(entity.getId(), null, postType);
		        for (V3xOrgRelationship rel : rels) {
		            if(rel.getObjective0Id() != null && !Strings.equals(rel.getObjective0Id(), -1L) && rel.getObjective1Id() != null && !Strings.equals(rel.getObjective1Id(), -1L)
		               && accessableAccounts.contains(rel.getOrgAccountId())){
		            	result.add(entity);
		            	break;
		            }
		...
		for(V3xOrgEntity entity : disableDatas){
			if(accessableAccounts.contains(entity.getOrgAccountId())){
				result.add(entity);
			}
			if(result.size()>=orgFastSearchLength){
	...
	public String getFastSearchDataStr(String key) throws BusinessException {
    	...
    	for(V3xOrgMember member :members){
			...
			}else if(member.getIsDeleted()){//添加删除的状态
                fsm.setText(member.getName()+"("+ResourceUtil.getString("common.button.delete.label")+")");
