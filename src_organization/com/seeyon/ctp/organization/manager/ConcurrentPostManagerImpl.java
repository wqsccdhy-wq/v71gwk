...
public class ConcurrentPostManagerImpl implements ConcurrentPostManager {
    ...
    public FlipInfo list4in(FlipInfo fi, Map params) throws BusinessException {
        ...
        Collections.sort(resultConcurrentPosts , new Comparator<WebV3xOrgConcurrentPost>(){
			public int compare(WebV3xOrgConcurrentPost o1, WebV3xOrgConcurrentPost o2) {
		        Long id1 = o1.getConSortId();
		        Long id2 = o2.getConSortId();
...
//        boolean isNullValue = false;//用于防护按主岗查询，主岗下没有人导致查询结果不正确
    ...
    @Override
    @CheckRoleAccess(roleTypes={Role_NAME.GroupAdmin,Role_NAME.HrAdmin})
    ...
    public void updateOne(Map map) throws BusinessException {
        ...
        privilegeMenuManager.updateInnerMemberMenuLastDate(member.getId(), Long.parseLong(String.valueOf(map.get("cAccountId"))));
/*        if(!rel.getOrgAccountId().equals(Long.valueOf(String.valueOf(map.get("cAccountId"))))) {
...
/*        V3xOrgRelationship r = new V3xOrgRelationship(rel);
    ...
    private void updateById(Long relId,Map map) throws BusinessException {
        //新建这个信息
        ...
        if(null != map.get("cDeptId")
                && Strings.isNotBlank(String.valueOf(map.get("cDeptId")))) {
        ...
        orgManagerDirect.updateConurrentPost(relId, m);
        ...
        privilegeMenuManager.updateInnerMemberMenuLastDate(member.getId(), cAccountId);
    }
    ...
    private void checkDupliateConpost(Map map) throws BusinessException {
        Long id = map.get("id") == null ? -1L : Long.valueOf(String.valueOf(map.get("id")));
        Long memberId = Long.valueOf(String.valueOf(map.get("cMemberId")));
        Long cAccountId = Long.valueOf(String.valueOf(map.get("cAccountId")));
        Long cPostId = null;
        if(null != map.get("cPostId")
...
/*            if(null != rel && null != rel.getObjective0Id()) {
    ...
    private void deleteConRoles(long memberId, long conAccountId) throws BusinessException {
        ...
        //orgDao.deleteOrgRelationshipPO(OrgConstants.RelationshipType.Member_Role.name(), memberId,conAccountId, objectiveIds);
        //緩存老不一致，單獨刪除下緩存
        ...
        for(V3xOrgRelationship rs: rsList){
        	Long roleId = rs.getObjective1Id();
        	if(roleId != null){
        		V3xOrgRole role = orgManager.getRoleById(roleId);
        		if(role != null && (role.getBond() == OrgConstants.ROLE_BOND.NULL1.ordinal() || role.getBond() == OrgConstants.ROLE_BOND.NULL2.ordinal())){
        			continue;
        		}
        	}
        ...
        orgDao.deleteOrgRelationshipPOs(orList);
...
//        }
        	...
        	if(createUser != null){
        		...
        		if(createUser.getIsAdmin()){
        			if(account.isGroup()){
        				...
        				userName = account.getName() + ResourceUtil.getString("org.account_form.adminName.value");//单位管理员
        			}
        		}else{
    ...
    public HashMap viewOne(Long relId) throws BusinessException {
        ...
        //主岗
        map.put("primaryPost","");
        Long postId = cMember.getOrgPostId();
        if(postId != null) {
        	V3xOrgPost post = orgManager.getPostById(postId);
        	if(post != null) {
        		if(isShowAccountName) {
        			map.put("primaryPost", post.getName()+"("+sAccount.getShortName()+")");
        		} else {
        			map.put("primaryPost", post.getName());
        		}
        	}
        }
...
//        }
    ...
    public List<WebV3xOrgSecondPost> dealSecondPostInfo(Map<Long, List<MemberPost>> resultMap) throws BusinessException {
        ...
        for (Long memberId : resultMap.keySet()) {
            ...
            if(secondPosts.size() > 3) {
                ...
                StringBuilder post2Str = new StringBuilder();
	...
	public String getIncludeUnitIds() throws BusinessException {
		...
		if(currentUnitId == -1){
			currentUnitId = orgManager.getRootAccount().getId();
		}
        ...
        StringBuilder excludeUnitIDs = new StringBuilder();
        for(V3xOrgAccount unit : subUnitList){
	...
	public String checkCanDel(Long relId) throws BusinessException {
        ...
        if(null != relPO){
        	...
        	String includeUnitId = getIncludeUnitIds();
        	if(Strings.isNotBlank(includeUnitId) && includeUnitId.indexOf(targetUnitId+"") == -1){
        		return ResourceUtil.getString("org.validate.check1");
        	}
        }
		return "";
	}
