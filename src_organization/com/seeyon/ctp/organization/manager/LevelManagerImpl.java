...
public class LevelManagerImpl implements LevelManager {
	...
	public Long saveLevel(String accountId, Map level) throws BusinessException {
        ...
        if (newlevel.getId() == null) {
            ...
            //记录日志
            appLogManager.insertLog4Account(user, user.isGroupAdmin() ? accId : AppContext.currentAccountId(), AppLogAction.Organization_NewLevel, user.getName(), newlevel.getName());
        } else {
            OrganizationMessage m = orgManagerDirect.updateLevel(newlevel);
            OrgHelper.throwBusinessExceptionTools(m);
            //记录日志
            appLogManager.insertLog4Account(user, user.isGroupAdmin() ? accId : AppContext.currentAccountId(), AppLogAction.Organization_UpdateLevel, user.getName(), newlevel.getName());
        }
	...
	public String deleteLevel(List<Map<String,Object>> level) throws BusinessException {
        ...
        //记录日志
        appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? levellist.get(0).getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_DeleteLevel, appLogs);
        return "";
	}
