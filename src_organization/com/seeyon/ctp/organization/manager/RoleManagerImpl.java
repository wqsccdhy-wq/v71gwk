...
public class RoleManagerImpl implements RoleManager {
    ...
    public void batchRole2Entity(Long roleId, String entityIds) throws BusinessException {
        ...
        //日志
        int bond = role.getBond();
        //管理员能看到的角色才记日志
    	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
    		if(user != null){
    			appLogManager.insertLog4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_RoleToMember,  user.getName(),role.getShowName(),oldRoleMembers,newRoleMembers);
    		}else{
    ...
    public Map<String,String> checkRoles(String roleIds, Long memberId, Long deptId) throws BusinessException{
    	...
    	for(String roleId : roleIdArr){
    		V3xOrgRole role = orgManager.getRoleById(Long.valueOf(roleId));
    		//校验部门用车管理员，一个人只能是一个部门的部门用车管理员
    		if(AppContext.hasPlugin("didicar") && OrgConstants.Role_NAME.DepartmentSpecialTrainAdmin.name().equals(role.getName())){
                EnumMap<RelationshipObjectiveName, Object> enummap = new EnumMap<RelationshipObjectiveName, Object>(
                        RelationshipObjectiveName.class);
                enummap.put(OrgConstants.RelationshipObjectiveName.objective1Id, role.getId());
                List<V3xOrgRelationship> rels = orgCache.getV3xOrgRelationship(
                ...
                if (CollectionUtils.isNotEmpty(rels)) {
                    if(rels.size()>1||rels.get(0).getObjective0Id().longValue()!=deptId){
                    	...
                    	result.put("info", ResourceUtil.getString("didicar.plugin.deptspecialmanager.onlyone", orgManager.getMemberById(memberId).getName()));
                        return result;
                    }
                }
    		}
	...
	public void copyrole(List<Map<String, Object>> rolelistmap, String type)
		...
		for (V3xOrgRole inv3xOrgRole : rolelist) {
			...
			//日志
	    	int bond = newv3xOrgRole.getBond();
	    	//管理员能看到的角色才记日志
	    	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
	    		appLogManager.insertLog4Account(user, user.isGroupAdmin() ? inv3xOrgRole.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_CopyRoles, user.getName(), newv3xOrgRole.getShowName());
	    	}
			//集团基准角色同步
...
//                            }
    ...
    public Long createRole(V3xOrgRole role) throws BusinessException {
    	...
    	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
    		if(null != user) {
    			appLogManager.insertLog4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_NewRole, user.getName(), role.getName());
    		}
    	}
...
//    	role.setCategory("1");
    	...
    	//管理员能看到的角色才记日志
    	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
    		appLogManager.insertLog4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_UpdateRole, user.getName(), role.getShowName());
    	}
    }
    ...
    public void deleteRole(Long[] roles) throws BusinessException {
        if (roles != null && roles.length>0) {
        	...
        	//管理员能看到的角色才记日志
        	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
        		appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_DeleteRole, appLogs);
        	}
        }
    }
    @Override
    ...
    public void deleteRole(Long roleId,Boolean whetherEnforced) throws BusinessException {
    	...
    	//管理员能看到的角色才记日志
    	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
    		appLogManager.insertLog4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_DeleteRole, appLog);
    	}
    }
    ...
    public void disenableRole(Long[] roles) throws BusinessException {
        if (roles != null) {
        	...
        	//管理员能看到的角色才记日志
        	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
        		appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? accountId : AppContext.currentAccountId(), AppLogAction.Organization_DisEnableRole, appLogs);
        	}
        }
    }
    @Override
    ...
    public void enableRole(Long[] roles) throws BusinessException {
        if (roles != null) {
            ...
            //日志
        	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
        		appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? accountId : AppContext.currentAccountId(), AppLogAction.Organization_EnableRole, appLogs);
        	}
        }
    }
    @Override
...
//    	}
        ...
        if(v3xOrgRole!=null && v3xOrgRole.isValid()){
            ...
            for (V3xOrgEntity v3xOrgEntity : entitys) {
                ...
                for (V3xOrgMember v3xOrgMember : members) {
                    ...
                    map.put("loginName", v3xOrgMember.getLoginName());
    ...
    public void SycGroupRoleAdd(V3xOrgRole role) throws BusinessException{
    	...
    	for (OrgUnit orgUnit : allAccounts) {
    		if(orgUnit.getExternalType() != OrgConstants.ExternalType.Inner.ordinal()) {
    			continue;
    		}
    ...
    public Long updateRole(V3xOrgRole role) throws BusinessException {
        ...
        if (role != null && role.getId() != null) {
            ...
            if(orgManager.getAccountById(role.getOrgAccountId()).isGroup()){
                ...
                List<OrgRole> list = orgDao.getAllRolePO(null, true, m, null);
                Set<Long> s = new HashSet<Long>();
                if (list.size() > 1) {
                    for (OrgRole l:list) {
                        if (!role.getId().equals(l.getId())) {
                            s.add(l.getId());
                        }
                    }
                }
            	...
            	for (V3xOrgRelationship v3xOrgRelationship : rellist) {
                    ...
                    if (s.contains(accrole.getId())) {
                        s.remove(accrole.getId());
                    }
    			}
            	...
            	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
            		...
            		appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_UpdateRole, appLogs);
            	}
                ...
                if (!s.isEmpty()) {
                    for (Long rId:s) {
                        V3xOrgRelationship newrel = new V3xOrgRelationship();
                        newrel.setId(UUIDLong.longUUID());
                        newrel.setSourceId(role.getId());
                        newrel.setObjective0Id(rId);
                        newrel.setKey(OrgConstants.RelationshipType.Banchmark_Role.name());
                        orgManagerDirect.addOrgRelationship(newrel);
                    }
                }
            } else if (RoleTypeEnum.relativeflag.getKey() == role.getType()){
                List<V3xOrgRelationship> rellist = orgManager.getV3xOrgRelationship(OrgConstants.RelationshipType.Banchmark_Role, role.getId(), null, null);
                if (rellist.isEmpty()) {
                    Map<String,Object> m = new HashMap<String, Object>();
                    m.put("code", role.getCode());
                    List<OrgRole> list = orgDao.getAllRolePO(OrgConstants.GROUPID, true, m, null);
                    
                    if (Strings.isNotEmpty(list)) {
                        V3xOrgRelationship newrel = new V3xOrgRelationship();
                        newrel.setId(UUIDLong.longUUID());
                        newrel.setSourceId(list.get(0).getId());
                        newrel.setObjective0Id(role.getId());
                        newrel.setKey(OrgConstants.RelationshipType.Banchmark_Role.name());
                        orgManagerDirect.addOrgRelationship(newrel);
                    }
    ...
    public void updateRoleResource(List<PrivTreeNodeBO> nodes, Long roleId) throws BusinessException {
        updateRoleResource(nodes, roleId, true);
    }
    
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    @CheckRoleAccess(roleTypes={Role_NAME.GroupAdmin,Role_NAME.AccountAdministrator,Role_NAME.HrAdmin})
    public void updateRoleResource(List nodes, Long roleId, boolean reset) throws BusinessException {
        ...
        Set<Long> UnallowGovdocMenuIds = getUnallowGovdocMenuIds(nodes,roleId);
        for (int i = 0; i < nodes.size(); i++) {
            ...
            nodeId = node.getIdKey();
             if (nodeId.indexOf(PrivTreeNodeBO.menuflag) != -1) {
                ...
                if (menuBO != null) {
                	if(UnallowGovdocMenuIds.contains(menuId)) {
                		continue;
                	}
                    roleRes = new PrivRoleMenu();
                    roleRes.setNewId();
...
//        }
            ...
            //管理员能看到的角色才记日志
        	if(bond == OrgConstants.ROLE_BOND.GROUP.ordinal() || bond == OrgConstants.ROLE_BOND.ACCOUNT.ordinal() || bond == OrgConstants.ROLE_BOND.DEPARTMENT.ordinal()){
        		appLogManager.insertLog4Account(user, user.isGroupAdmin() ? role.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_RoleToResource, user.getName(), role.getShowName(),
...
//        System.out.println("总耗时为："+(endMili-startMili)+"毫秒");
    ...
    public String checkGovdocRoleResource(List nodes, Long roleId) throws BusinessException {
    	String result = "";
    	Set<Long> UnallowGovdocMenuIds = getUnallowGovdocMenuIds(nodes,roleId);
    	if(Strings.isNotEmpty(UnallowGovdocMenuIds)) {
    		for(Long menuId : UnallowGovdocMenuIds) {
    			PrivMenuBO menuBO = privilegeMenuManager.findById(menuId);
        		if(Strings.isEmpty(result)) {
        			result = ResourceUtil.getString(menuBO.getName());
        		}else {
        			result = result + "、" + ResourceUtil.getString(menuBO.getName());
        		}
    		}
    	}
    	if(Strings.isNotBlank(result)) {
    		result = ResourceUtil.getString("member.role.checkgovdoc.menu",result);
    	}
    	return result;
    }
    
    private Set<Long> getUnallowGovdocMenuIds(List nodes, Long roleId) throws BusinessException{
    	Set<Long> result = new HashSet<Long>();
    	V3xOrgRole role = orgManager.getRoleById(roleId);
    	if(role != null) {
    		Set<Long> checkMenuIds = new HashSet<Long>();
    		checkMenuIds.add(-1984192967792833848L); //发文拟文，老菜单的id
    		checkMenuIds.add(2718897185706164469L); //发文拟文,新菜单的id
    		
    		checkMenuIds.add(-6954715937605598651L); //签报拟文
    		checkMenuIds.add(328301195683972076L);  //收文登记
            ...
            for (int i = 0; i < nodes.size(); i++) {
            	PrivTreeNodeBO node = (PrivTreeNodeBO) nodes.get(i);
                String nodeId= node.getIdKey();
                 if (nodeId.indexOf(PrivTreeNodeBO.menuflag) != -1) {
                    // 勾选菜单时自动将入口资源勾选
                    Long menuId = Long.parseLong(node.getIdKey().replace(PrivTreeNodeBO.menuflag, ""));
                    PrivMenuBO menuBO = privilegeMenuManager.findById(menuId);
                    if (menuBO != null) {
                    	if(checkMenuIds.contains(menuId)){
                    		result.add(menuId);
                    	}
