...
/*        for(PrivPermission privPerm : allPrivPerm){
    ...
    @Override
    @CheckRoleAccess(roleTypes={Role_NAME.GroupAdmin})
    public void setZXPermissionType(String type) throws BusinessException, CloneNotSupportedException{
    	ConfigItem item = new ConfigItem();
...
//    	if ("2".equals(type)) {
//    		item = getConfigManager().getConfigItem("LIC_ZX_PERMISSION_TYPE", "LIC_ZX_PERMISSION_TYPE");
//    		List<PrivPermission> privPermissions = licensePerMissionCache.getAllPerMissionPO();
//    		List<PrivPermission> newPrivPermissions = new ArrayList<PrivPermission>();
//    		for (PrivPermission privPermission : privPermissions) {
//    			if (privPermission.getLictype().equals(3)) {
//    				V3xOrgAccount account = Optional.of(orgManager.getAccountById(privPermission.getOrgAccountId())).get();
//    				if (!account.isGroup()) {
//    					int realCount = queryZXAuthCount(privPermission.getOrgAccountId());
//        				if (realCount > privPermission.getDistributionnum()) {
//        					PrivPermission clonePrivPermission = (PrivPermission) privPermission.clone();
//        					clonePrivPermission.setDistributionnum(Long.valueOf(realCount));
//        					newPrivPermissions.add(clonePrivPermission);
//        				}
//    				}
//    			}
//    		}
//    		licensePerMissionDao.saveOrUpdatePerMissionPO(newPrivPermissions);
//    	}
    }
	...
	public Map<String, String> getPermissionInfo(String accId) throws BusinessException {
		...
		map.put("serverversion",this.getVersion());
...
/*		map.put("usem1numname", ResourceUtil.getString("licensePermission.all")+ResourceUtil.getString("licensePermission.use")+MxVersion+ResourceUtil.getString("licensePermission.num")+":");
		...
		if(Strings.isNotBlank(accId) && !OrgConstants.GROUPID.toString().equals(accId)){
			...
			String m1PermissionType = getM1PermissionType();
			//如果单位的已使用许可数大于分配的许可数，就按照已经使用的许可数来计算。
			Map<Long,Integer> MAuthedMap = queryAuthedUserCount();
			List<V3xOrgAccount> allAccounts = getOrgManager().getAllAccounts();
			for(V3xOrgAccount account : allAccounts){
				Long accountId = account.getId();
				PrivPermission p = licensePerMissionDao.getPerMissionPO(accountId,2);
				Integer accountUsedNum = MAuthedMap.get(accountId);
				if("1".equals(m1PermissionType)){//mx 不控制许可数，直接取各个单位实际已经分配的许可数。
					if(accountUsedNum!=null){
						usem1num+=accountUsedNum;
					}
				}else{//mx 控制许可数，比较集团管理员给各个单位分配的授权数  和 各单位自己实际已经分配的授权数，哪个大，就取哪个数。
					boolean isNew = false;
					if(p == null || p.getId() == null) {
						isNew = true;
						p = new PrivPermission();
						p.setIdIfNew();
						p.setLictype(2);
						Object info = MclclzUtil.invoke(c1, "getInstance",new Class[]{String.class},null,new Object[]{String.valueOf(accountId)});
						p.setType(Integer.valueOf(MclclzUtil.invoke(c1, "getm1Type",null,info,null).toString()));
						p.setOrgAccountId(account.getId());
						p.setDistributionnum(0L);
					}
					if(accountUsedNum != null && accountUsedNum > p.getDistributionnum()){
						p.setDistributionnum(Long.valueOf(accountUsedNum));
						licensePerMissionDao.savePerMissionPO(p);
						usem1num+=accountUsedNum;
					}else{
						if(isNew){
							licensePerMissionDao.savePerMissionPO(p);
						}
						usem1num+=p.getDistributionnum();
					}
				}
			}
		...
		if (SystemEnvironment.hasPlugin("zx") && "private".equals(uc_deployment)) {
			...
			if (isRoot) {
				...
				if ("1".equals(getZXPermissionType())) {
					if (mcZX != null && !mcZX.equals(0)) {
						distributionzxsum += (Integer) mcZX;
					} else { //如果客户加密狗中没有授权数，默认给100授权数
						distributionzxsum += 100;
					}
				} else {
	...
	public void savePermissionDisInfo(Map perinfo) throws BusinessException {
		...
		for (Object object : keyset) {
			...
			if(privPermission.getLictype().equals(LicenseConst.PERMISSION_TYPE_SERVER)&&"2".equals(getServerPermissionType())&&((Long)MclclzUtil.invoke(c1, "getUseservernum",null,info,null)).intValue()>privPermission.getDistributionnum()){
				throw new BusinessException(ResourceUtil.getString("org.validate.permission3",getOrgManager().getAccountById(privPermission.getOrgAccountId()).getName()));
			}
			...
			if("M3".equals(MxVersion)){
					...
					if(privPermission.getLictype().equals(LicenseConst.PERMISSION_TYPE_M1)&&"2".equals(getM1PermissionType())){
						...
						if(authedCount != null){
							if(authedCount > privPermission.getDistributionnum()){
								throw new BusinessException(ResourceUtil.getString("org.validate.permission4",getOrgManager().getAccountById(privPermission.getOrgAccountId()).getName()));
							}
						}
					}
				}
				...
				if(privPermission.getLictype().equals(LicenseConst.PERMISSION_TYPE_M1)&&"2".equals(getM1PermissionType())&&((Long)MclclzUtil.invoke(c1, "getUsem1num",null,info,null)).intValue()>privPermission.getDistributionnum()){
					throw new BusinessException(ResourceUtil.getString("org.validate.permission5",getOrgManager().getAccountById(privPermission.getOrgAccountId()).getName()));
				}
			}
			
			//如果是致信，那么要求1、授权数量小于加密狗的授权数量 2、授权量大于该单位已经授权的人员数量
			if (privPermission.getLictype().intValue() == 3 && "2".equals(getZXPermissionType())) {
				if (Integer.parseInt(num) < queryZXAuthCount(Long.valueOf(str[1]))) {
				...
				if (mcZX != null && !mcZX.equals(0)) {
					if (queryZXAuthCount(Long.valueOf(str[1])) > (Integer) mcZX) {
						throw new BusinessException("单位："+getOrgManager().getAccountById(privPermission.getOrgAccountId()).getName()+"的致信的分配许可数大于可用许可数，请调整！");
					}
				} else {//如果客户加密狗中没有授权数，默认给100授权数
		...
		if(totalservernum>((Long)MclclzUtil.invoke(c1, "getTotalservernum",null,info,null)).intValue()){
			throw new BusinessException(ResourceUtil.getString("org.validate.permission7"));
		}
		if(totalm1num>((Long)MclclzUtil.invoke(c1, "getTotalm1num",null,info,null)).intValue()){
			throw new BusinessException(ResourceUtil.getString("org.validate.permission8"));
		}
		//如果大于致信注册总数，抛异常
		if (SystemEnvironment.hasPlugin("zx") && "private".equals(uc_deployment)) {
			Object mcZX = MclclzUtil.invoke(c2, "getRongCloudRegisterSize");
			if (mcZX != null && !mcZX.equals(0)) {
				if(totalzxnum > (Integer) MclclzUtil.invoke(c2, "getRongCloudRegisterSize")){
					throw new BusinessException(ResourceUtil.getString("org.validate.permission9"));
				}
			} else {//如果客户加密狗中没有授权数，默认给100授权数
				if(totalzxnum > 100){
					throw new BusinessException(ResourceUtil.getString("org.validate.permission9"));
	...
	private Map<Long,Integer> queryAuthedUserCount(){
		...
		try {
			...
			List<Map<String, Object>> list = agent.resultSetToList();
			if(Strings.isNotEmpty(list)){
				if(list.get(0).get("C")!=null || list.get(0).get("c")!=null){
					String c = list.get(0).get("C") != null ? list.get(0).get("C").toString() : (list.get(0).get("c") != null ? list.get(0).get("c").toString() : "");
					if(Strings.isNotBlank(c) && !"null".equals(c)){
						for(Map map : list){
							result.put(
									Long.valueOf(map.get("ORG_ID") != null ? map.get("ORG_ID").toString() : (map.get("org_id") != null ? map.get("org_id").toString() : "0")), 
									Integer.valueOf(map.get("C") != null ? map.get("C").toString() : (map.get("c") != null ? map.get("c").toString() : "0")));
						}
					}
				}
			}
