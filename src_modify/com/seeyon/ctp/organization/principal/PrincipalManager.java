...
public interface PrincipalManager {
	 ...
	 * @param loginName
	 * @return
	 * @throws NoSuchPrincipalException 如果用户名不存在
	 */
	 ...
	 * @param memberId
	 * @return
	 * @throws NoSuchPrincipalException 如果用户不存在
	 */
	 ...
	 * 添加一个登录身份
	 * 
	 * @param principal memberId,loginName,password三个字段必须都要有值，password必须是明文
	 ...
	 * 修改一个登录身份
	 * 
	 * @param principal memberId,loginName,password三个字段必须都要有值，password必须是明文或默认密码
	 */
	 ...
	 * @param loginName 登录名，如system、audit-admin。
	 * @param password 新密码
	 * @param isExpirationDate 是否要计算密码的超期时间，如果当前操作者修改的是自己的密码，就传false；否则传true，表示密码已经超期，登录进来后立即提示修改密码
	 * @return 修改成功返回<tt>true</tt>，否则返回<tt>false</tt>。
	 * @throws NoSuchPrincipalException 指定用户不存在抛出。
	 */
	 ...
	 * 批量添加登录身份
	 * 
	 * @param principals memberId,loginName,password三个字段必须都要有值，password必须是明文或默认密码
	 * @throws ExistPrincipalException
	 *             如果用户名已经存在
	 */
	...
	/**
	 * 批量修改用户的密码过期时间
	 * @param days //修改后的密码超期时间与之前的密码超期时间相差的天数
	 */
