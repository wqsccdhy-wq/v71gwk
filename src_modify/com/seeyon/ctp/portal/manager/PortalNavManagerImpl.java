...
public class PortalNavManagerImpl extends AsynchronousBatchTask<Map<String, Object>> implements PortalNavManager {
        ...
        if (list != null && list.size() > 0) {
            ...
            for (PortalNav nav : list) {
                try {
                    ...
                    String key =
                        nav.getNavType() + "_" + nav.getPortalId() + "_" + nav.getEntityId() + "_" + nav.getAccountId();
    ...
    public void appendPortal(PortalSet portal, String auth) throws BusinessException {
        ...
        // if(1!=portal.getPcPortal() || 2==portal.getPortalType() || 3==portal.getPortalType()||
        // 4==portal.getPortalType()) {
        // return ;//不是pc门户不追加导航
        // }
    ...
    public void savePortalNav(List<Map<String, String>> navs, String portalId) throws BusinessException {
        ...
        if (!user.isAdmin()) {
            CtpCustomize spaceCustom =
                customizeManager.getCustomizeInfoWithCache(user.getId(), CustomizeConstants.SPACE_ORDER);
            if (spaceCustom != null) {
    ...
    // private List<Map<String, String>> appendDefaultSpace1(List<Map<String, String>> list,String portalId) throws
    // BusinessException{
    // if(list==null){
    ...
    private List<PortalNavBo> checkAdminPriv(User user, List<PortalNavBo> list, String portalId)
        ...
        for (PortalNavBo bo : list) {
            if (idAndNames.containsKey(bo.getId()) || PortalNavType.linkProject.name().equals(bo.getNavType())
                || PortalNavType.linkSystem.name().equals(bo.getNavType())
                || (bo.getNavType() != null && bo.getNavType().startsWith(PortalNavType.thirdPartyPortal.name()))) {
    ...
    private Map<String, String> getAdminIdSet(List<PortalNavBo> spaces, List<PortalSet> portals, List<PrivMenuBO> menus,
        List<PortalLinkSpace> linkSpaceList) {
    ...
    private List<PortalNavBo> getAdminDefaultNav(User user, String portalIdStr, Long defaultSpaceId,
        boolean isMasterPortal) throws BusinessException {
                ...
                if (isMasterPortal) {
                    ...
                    if (defaultSpaceIndex >= 0) {
                        ...
                        if (defaultSpaceId != null) {
                            ...
                            // 去掉图标的/seeyon
                            if (Strings.isNotBlank(spaceIcon) && spaceIcon.indexOf("fileUpload.do") > -1
                                && spaceIcon.startsWith("/seeyon")) {
                                ...
                                if (menuid != null) {
                                    ...
                                    if (byId != null) {
                                        ...
                                        if ("portal".equals(byId.getModuleFrom())) {
                                            // 综合业务菜单不展示自定义标识
                                            navBo.setNavName("[" + ResourceUtil.getString("portalcustomizemenu.custom")
                                                + "]" + navName);
                                        }
                                    }
                                }
    ...
    private List<Long> getDomainIds(User user) throws BusinessException {
        ...
        if (member.getIsInternal()) {
            ...
            return orgManager.getUserDomainIDs(member.getId(), user.getLoginAccount(), V3xOrgEntity.ORGENT_TYPE_MEMBER,
                V3xOrgEntity.ORGENT_TYPE_DEPARTMENT, V3xOrgEntity.ORGENT_TYPE_TEAM);
        }
    }
    ...
    public PortalSpaceFix getDefaultSpace(DefaultSpaceSetting defaultSpace) throws BusinessException {
        ...
        if (defaultSpace == null) {
            fix = portalCacheManager.getDefaulPortalSpaceFixFromCache(Constants.SpaceType.Default_personal.ordinal(),
            ...
            if (Strings.isBlank(spaceId)) {
                ...
                if (DefaultSpaceSetting.Default_Space_Personal_Type.equals(spaceType)) {
                    if (user.isInternal()) {
                        fix = portalCacheManager.getDefaulPortalSpaceFixFromCache(
                            Constants.SpaceType.Default_personal.ordinal(), user.getLoginAccount());
                    } else {
                        fix = portalCacheManager.getDefaulPortalSpaceFixFromCache(
                            Constants.SpaceType.Default_out_personal.ordinal(), user.getLoginAccount());
                    }
                } else if (DefaultSpaceSetting.Default_Space_Group_Type.equals(spaceType)) {
                    fix = portalCacheManager.getDefaulPortalSpaceFixFromCache(Constants.SpaceType.group.ordinal(),
                        OrgConstants.GROUPID);
                } else if (DefaultSpaceSetting.Default_Space_Corporation_Type.equals(spaceType)) {
                    fix = portalCacheManager.getDefaulPortalSpaceFixFromCache(Constants.SpaceType.corporation.ordinal(),
                        user.getLoginAccount());
                }
            } else {
    ...
    private List<PortalNavBo> sortSpaces(List<PortalSpaceFix> spaces, List<ThirdpartySpace> thirdPartySpacesList)
        throws BusinessException {
        List<PortalNavBo> result = new ArrayList<PortalNavBo>();
        ...
        for (PortalSpaceFix space : spaces) {
            ...
            if (unitId != accountId && space.getType() != SpaceType.group.ordinal()
                && space.getType() != SpaceType.public_custom_group.ordinal()) {
        ...
        if (thirdPartySpacesList != null && thirdPartySpacesList.size() > 0) {
            for (ThirdpartySpace space : thirdPartySpacesList) {
                ...
                String spacePath =
                    "/seeyon/thirdpartyController.do?method=show&id=" + space.getId() + "&pageUrl=" + pageUrl;
                bo.setUrl(spacePath);
                sort++;
                result.add(bo);
            }
    ...
    public List<PortalNavBo> getSpaceForNav(String portalId) throws BusinessException {
        ...
        if (isMasterPortal || portalLongId == null) {// OA-148929 ,空间卡片下没有第三方空间
            try {
                thirdPartySpacesList = ThirdpartySpaceManager.getInstance().getAccessSpaces(orgManager,
                    AppContext.currentUserId(), null, null);
            } catch (Exception e) {
        ...
        if (!user.isAdmin()) {
            ...
            V3xOrgMember member = orgManager.getMemberById(user.getId());
            if (!member.getIsInternal() && !member.isScreenGuest() && !member.isDefaultGuest()
                && (isMasterPortal || portalLongId == null)) {
    ...
    private String[] getDefaultSpaceIconAndNameByType(String sType, String spaceName, String spaceAccountId,
        Long accountId, SpaceBO space) {
    ...
    private List<PortalNavBo> converPortalNavBo(String portalId, List<PortalNavBo> list, String containMenu,
        String needSpaceGroup, String needPortalGroup, Set<String> ids, Map<String, PortalSet> portalMap,
        Map<String, SpaceBO> spaceMap, Map<String, PrivMenuBO> menuMap, boolean isDefault, String needTileSecondMenu)
        throws BusinessException {
        ...
        // 兼容老版本导航用
        CtpCustomize custom =
            this.customizeManager.getCustomizeInfoWithCache(user.getId(), CustomizeConstants.SPACE_ORDER);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (Strings.isNotBlank(headerspaceId) && Strings.isDigits(headerspaceId)) {
                if (!ids.contains(headerspaceId)) {
                    ...
                    PortalSpaceFix headerSpace =
                        portalCacheManager.getPageFixIdCacheByKey(Long.parseLong(headerspaceId));
            ...
            if ("portal".equals(navType)) {
                ...
                if (space != null) {
                    ...
                    String[] nameAndIcon = this.getDefaultSpaceIconAndNameByType(space.getSpaceType(), spaceName,
                        space.getAccountId(), user.getLoginAccount(), space);
                    ...
                    if (String.valueOf(SpaceType.related_system.ordinal()).equals(space.getSpaceType())
                        && (Strings.isNotBlank(space.getSpaceIcon())
                            && space.getSpaceIcon().indexOf("fileUpload.do") == -1)) {
                    ...
                    if (!String.valueOf(Constants.SpaceType.thirdparty.ordinal()).equals(space.getSpaceType())
                        ...
                        if (isMasterPortal) {
                            ...
                            if (systemPortal != null
                                && (systemPortal.getId().longValue() == 1 || (systemPortal.getParentId() != null
                                    && systemPortal.getParentId().longValue() == 1))) {
                        ...
                        if (spaceBelongPortalId.equals(portalId) || (isMasterPortal && "1".equals(spaceBelongPortalId))
                            ...
                            if (String.valueOf(SpaceType.v_report.ordinal()).equals(space.getSpaceType())) {// 如果是报表空间url和普通的逻辑不一样
                                ...
                                if (!isMasterPortal) {// 如果子门户
                                    ...
                                    Long id = Strings.isNotBlank(spaceBelongPortalId)
                                        ? Long.parseLong(spaceBelongPortalId) : null;
                                    if (id != null) {
                                        PortalSet set = portalCacheManager.getPortalSetFromCache(id);
                                        if (id.longValue() == 1l
                                            || (set.getParentId() != null && set.getParentId().longValue() == 1l)) {
                    ...
                    if ("1".equals(needSpaceGroup)
                        && (spaceBelongPortalId.equals(portalId) || (isMasterPortal && "1".equals(spaceBelongPortalId))
                            || (isMasterPortal
                                && (String.valueOf(SpaceType.thirdparty.ordinal()).equals(space.getSpaceType())
                                    ...
                                    || String.valueOf(SpaceType.related_project_space.ordinal())
                                        .equals(space.getSpaceType()))))) {
                        // 如果需要给当前门户空间分组显示
                        spaceGroup.add(nav);
                        continue;
                    }
    ...
    private void putPortals(List<PortalNavBo> boList, User user) throws BusinessException {
        ...
        List<PortalSet> portals = portalSetDao.getGroupAndUnitPortalNoMaster(OrgConstants.GROUPID,
            user.getLoginAccount(), this.getDomainIds(user));
        if (portals != null) {
    ...
    private Set<String> putPrivIds(User user, boolean isDefault, Map<String, PortalSet> portalMap,
        Map<String, SpaceBO> spaceMap, Map<String, PrivMenuBO> menuMap) throws BusinessException {
    ...
    private List<PortalNavBo> addNewNavForAdmin(User user, String portalId, List<PortalNavBo> list,
        boolean isMasterPortal) throws BusinessException {
        List<PortalNavBo> result = new ArrayList<PortalNavBo>();
        if (list == null || list.isEmpty()) {
            return result;
    ...
    public List<PortalNavBo> getCurrentUserNav(String portalId, String containMenu, String needSpaceGroup,
        ...
        DefaultSpaceSetting accountSetting =
            DefaultSpaceSetting.getDefaultSpaceSettingForAccount(AppContext.currentAccountId(), portalId);
        ...
        list = converPortalNavBo(portalId, list, containMenu, needSpaceGroup, needPortalGroup, ids, portalMap, spaceMap,
            menuMap, isDefault, needTileSecondMenu);
        ...
        if (Strings.isNotBlank(canChangeDefaultSpace) && !isSubPortal) {
            if ("1".equals(needSpaceGroup)) {
                if (list != null && !list.isEmpty()) {
                    ...
                    spaceList = sortNav(ids, spaceList, accountSetting, canChangeDefaultSpace, "nav", portalId,
                        spaceMap, isDefault);
        ...
        if (list != null && list.size() > 0) {
            ...
            try {
                if (navData == null && isDefault && unitIndex != -1 && groupIndex != -1 && (unitIndex < groupIndex)
                    && (!("0".equals(canChangeDefaultSpace)))) {
                    PortalSpaceFix defaultSpace = this.getDefaultSpace(accountSetting);
                    boolean isCorporationSpaceDefault = false;
                    isCorporationSpaceDefault = (defaultSpace != null && defaultSpace.getType() != null
                        && (defaultSpace.getType() == SpaceType.corporation.ordinal()));
    ...
    @Override
    @AjaxAccess
    public Map<String, Object> getCurrentUserNavPage(String portalId, String containMenu, String needSpaceGroup,
        String needPortalGroup, String needTileSecondMenu, Integer pageSize, Integer pageNum) throws BusinessException {
        Map<String, Object> dataMap = new HashMap<String, Object>(2);
        List<PortalNavBo> portalAllNavs =
            getCurrentUserNav(portalId, containMenu, needSpaceGroup, needPortalGroup, needTileSecondMenu);// 所有的导航
    ...
    // 硬把默认空间怼到第一位
    private List<PortalNavBo> sortNav(Set<String> ids, List<PortalNavBo> list, DefaultSpaceSetting defaultSpace,
        String canChangeDefaultSpace, String navType, String portalId, Map<String, SpaceBO> spaceMap, boolean isDefault)
        throws BusinessException {
        List<PortalNavBo> result = new ArrayList<PortalNavBo>();
        if (list != null) {
            ...
            if (defaultSpaceFix == null || !ids.contains(defaultSpaceFix.getId().toString())) {// 默认空间也要经过权限控制才行fix bug
                                                                                               // OA-141325
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                // 是否可以在导航数据中找到默认空间
                if (hasDefaultSpaceRight && (bo.getId().equals(defaultSpaceFix.getId().toString())
                    || (space != null && space.getParentId() != null
                        && defaultSpaceFix.getId().longValue() == space.getParentId().longValue()))) {
                    index = i;
                    beFind = true;
                }
                if (null != space) {
                    ...
                    if (space.getType() == SpaceType.Default_personal.ordinal()
                        || space.getType() == SpaceType.personal.ordinal()) {// 个人空间
                        personIndex = i;
                    }
                    if (space.getType() == SpaceType.outer.ordinal()
                        || space.getType() == SpaceType.Default_out_personal.ordinal()) {// 编外人员空间
                        extIndex = i;
                    }
                }
            ...
            if (extIndex > -1) {// 有外部人员空间
                ...
                if (needReplacePersonalSpaceWithLeaderSpace && personIndex > -1 && pushedIndex > -1 && isDefault) {
                    ...
                    if (!needReplacePersonalSpaceWithLeaderSpace && hasDefaultSpaceRight && !beFind
                        && !"1".equals(canChangeDefaultSpace) && !firstIsLeaderSpace) {
    ...
    public List<PortalNavBo> getCurrentUserRelatedProjectSpace() throws BusinessException {
        ...
        if (CollectionUtils.isNotEmpty(relatedProjectList)) {
            for (ProjectBO rpj : relatedProjectList) {
                PortalNavBo bo = new PortalNavBo(String.valueOf(rpj.getId()), Strings.toHTML(rpj.getProjectName()),
                    PortalNavType.linkProject.name(), "mainfrm",
                    "/seeyon/project/project.do?method=projectSpace&from=fromSpace&projectId=" + rpj.getId(),
                    "menu_relateproject.png", 0, null, String.valueOf(SpaceType.related_project_space.ordinal()));
    ...
    public List<PortalNavBo> getLinkSpaces() throws BusinessException {
        ...
        for (PortalLinkSpace pls : list) {
            if (user.isAdmin()) {
                PortalNavBo bo =
                    new PortalNavBo(String.valueOf(pls.getId()), pls.getSpaceName(), PortalNavType.linkSystem.name(),
                        "mainfrm", "/seeyon/portal/linkSystemController.do?method=linkConnect&linkId=" + pls.getId(),
                        "menu_linksystem.png", 0, null, String.valueOf(SpaceType.related_system.ordinal()));
                result.add(bo);
            } else {
                ...
                if (hasSec) {
                    PortalNavBo bo = new PortalNavBo(String.valueOf(pls.getId()), pls.getSpaceName(), "linkSystem",
                        "mainfrm", "/seeyon/portal/linkSystemController.do?method=linkConnect&linkId=" + pls.getId(),
                        "menu_linksystem.png", 0, null, String.valueOf(SpaceType.related_system.ordinal()));
                    result.add(bo);
                }
    ...
    public void savePortalMenus(String menuJson, String portalId, Long entityId, Long accountId)
        throws BusinessException {
    ...
    @Override
    public void pushLeaderNav(PortalSpaceFix space, List<V3xOrgMember> members, Long accountId)
        throws BusinessException {
            ...
            if ("space".equals(nav.getNavType()) && Strings.isNotBlank(id)) {
                ...
                if (fixId != null) {
                    ...
                    if (fix != null) {
                        ...
                        if (Constants.SpaceType.leader.ordinal() == spaceType
                            || Constants.SpaceType.default_leader.ordinal() == spaceType) {
                            // 如果导航里已存在则去掉从新推
                            if (fixId.longValue() == space.getId().longValue() || (fix.getParentId() != null
                                && fix.getParentId().longValue() == space.getId().longValue())) {
                                continue;
                            }
                        }
                        // 如果是个人空间或者是外部人员空间则去掉 如果是当前推送的领导空间去重(第一位已经放了一个)
                        if (Constants.SpaceType.Default_personal.ordinal() == spaceType
                            || Constants.SpaceType.personal.ordinal() == spaceType
                            || Constants.SpaceType.Default_out_personal.ordinal() == spaceType
                            || Constants.SpaceType.outer.ordinal() == spaceType) {
                            nav.setIsDelete("1");
    ...
    @Override
    public PortalNav getDefaultNav(Long entityId, Long accountId, String portalId, String navType)
        throws BusinessException {
    ...
    @Override
    public void appendThirdPartyToMasterPortalNav(String portalType, PortalNavBo nav, List<Long> memberIds)
        throws BusinessException {
        if (memberIds == null || memberIds.isEmpty() || nav == null || Strings.isBlank(nav.getId())
            || Strings.isBlank(nav.getNavName())) {
    ...
    private PortalNav newPortalNav(Long userId, Long accountId, String portalId, String navType,
        List<PortalNavBo> navs) {
        ...
        if (spaces != null && !spaces.isEmpty()) {
            for (SpaceBO space : spaces) {
                if (spaceId == null) {
                    ...
                    if (spaceId.toString().equals(space.getSpaceId())
                        || spaceId.toString().equals(space.getParentId())) {
                        return space.getSpacePath();
                    }
                }
            }
    ...
    private void saveOrAppendMobilePortalNav(String portalId, String mobileNavType, List<Map<String, String>> navs,
        String from) throws BusinessException {
        if (navs == null) {
            navs = new ArrayList<Map<String, String>>();
        }
        User user = AppContext.getCurrentUser();
    ...
    @Override
    public void saveMobilePortalNav(String portalId, String mobileNavType, List<Map<String, String>> navs)
        throws BusinessException {
    ...
    private void appendAllSpaceInPortalToNav(String portalId) throws BusinessException {
        ...
        if (portalSetFromCache != null) {
            // 门户下空间
            List<PortalSpaceFix> mobileSpacesList =
                spaceManager.getMobileSpace4Admin(portalSetFromCache.getAccountId(), "mobile", portalId);
        ...
        if (portalSetFromCache != null) {
            ...
            if (portalId.equals(mainPortalIdStr)) {
                ...
                myAccountSb.append(
                    " from PortalSet where id!=:id and accountId =:accountId and (portalType=:typeA or portalType=:typeB) order by sortId");
                ...
                sb.append(
                    " from PortalSet where accountId !=:accountId and (portalType=:typeA or portalType=:typeB) order by sortId");
                ...
                for (PortalSet realAppendSet : portalSetRealAppendList) {
                    List<PortalNavBo> portalNavBos = appendMobilePortalToMainPortal(realAppendSet,
                        "Account|" + portalSetFromCache.getAccountId(), true);
    ...
    private PortalNav getUserMobileNavCommon(String portalId, String mobileNavType, String fromType)
        throws BusinessException {
        PortalNav navByCache = getMobileNavByCache(portalId, mobileNavType);
        if ("mTopOrMiddleNav".equals(mobileNavType) && Strings.isNotBlank(portalId) && Strings.isDigits(portalId)
            && navByCache == null) {
            ...
            if (Strings.isNotBlank(allIds)) {
                if ("index".equals(bo.getNavType()) && allIds.endsWith("index")) {
                    ...
                    if (!AppContext.isAdmin()) {
                        if ("mBottomNav".equals(mobileNavType)) {
                            ...
                            if (datatplJsArr != null
                                && (datatplJsArr.contains("navBar") || datatplJsArr.contains("topNav"))) {
                                hasMiddleOrTopNav = true;
                            }
                            if (hasMiddleOrTopNav == true) {
                                PortalNav userMobileNavCommon =
                                    getUserMobileNavCommon(portalId, "mTopOrMiddleNav", "front");// 中上导航
                        ...
                        List<SpaceBO> spaces =
                            portalCacheManager.getSpaceBoList(AppContext.getCurrentUser(), "mobile", portalId);
                    ...
                    if (Strings.isNotBlank(id) && Strings.isDigits(id)) {
                        ...
                        if (Strings.isNotBlank(portalId) && Strings.isDigits(portalId)) {
                            ...
                            if (fromPortal != null && fromPortal.getAccountId() != null) {
                                ...
                                String mainPortalIdStr =
                                    portalCacheManager.getPortalGlobalConfigFromCache(item, accountId, accountId);// 配置portal对应的移动主门户
                                Long navAccountId = portalSetFromCache.getAccountId();
                                if (!portalId.equals(mainPortalIdStr) && (navAccountId != null)) {
                                    // 配置的门户对应的主门户
                                    String navMainPortalIdStr = portalCacheManager.getPortalGlobalConfigFromCache(item,
                                        navAccountId, navAccountId);// 配置portal对应的移动主门户
                                    if (id.equals(navMainPortalIdStr)
                                        && (navAccountId.longValue() != accountId.longValue())) {
                                        logger.warn("子门户" + portalId + ";导航" + id + "是其他单位的主门户，过滤掉不显示。" + navAccountId
                                            + ";" + accountId);
                                        continue;
                                    }
                                }
                ...
                if (!"append".equals(fromType)) {
                    ...
                    if (imagePortletLayoutReal != null) {
                        ...
                        if (Strings.isNotBlank(imagePortletLayoutRealId)
                            && imagePortletLayoutRealId.indexOf("singleURL|thirdPartyCategory") < 0) {
    ...
    @Override
    public List<PortalNavBo> appendMobilePortalToMainPortal(PortalSet portalSet, String canshare, boolean firstsave)
        throws BusinessException {
    ...
    private ImagePortletLayout getImagePortletLayoutReal(String allIds) {
        ...
        Map<String, BasePortletCategory> portletCreatersId =
            new HashMap<String, BasePortletCategory>(portletCreaters.size());
    ...
    private void appendThirdPortalToMasterPortalNav(Map<String, Object> paramMap) {
        try {
            List<Long> memberIds = paramMap.get("memberIds") == null ? new ArrayList<Long>()
                : ((ArrayList<Long>)paramMap.get("memberIds"));
    ...
    public PortalNavBo generatePortalNavBoFromSpaceBO(SpaceBO space) {
        ...
        String spaceIcon = space.getSpaceIcon();
        if (String.valueOf(SpaceType.related_system.ordinal()).equals(space.getSpaceType())
            && (Strings.isNotBlank(space.getSpaceIcon()) && space.getSpaceIcon().indexOf("fileUpload.do") == -1)) {
            spaceIcon = nameAndIcon[0];
        }
        if (Strings.isBlank(spaceIcon)) {
            spaceIcon = nameAndIcon[0];
     ...
     */
    @Override
    public List<PortalCustomizeMenu> createG6PortalNav(Long accountId, Long portalId, Long spaceId, Long createMemberId)
        throws BusinessException {
