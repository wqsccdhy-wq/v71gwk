...
public class GovdocGenerateManagerImpl implements GovdocGenerateManager {
    ...
    public void generateTemplateCategory(V3xOrgAccount account) throws BusinessException {
        // 复制新公文模板分类
        repairTemplateCategory(account);
    }

    private void repairTemplateCategory(V3xOrgAccount account) throws BusinessException {
        // 判断是否已有行政公文
        /*List<CtpTemplateCategory> list = templateManager.getCategoryByName("行政公文", account.getId());
        if(!list.isEmpty()){
        	return;
        }*/
        ...
        LOGGER.info("创建单位" + account.getName() + "，自动新建新发文下级行政公文子分类结束...");
        ...
        LOGGER.info("创建单位" + account.getName() + "，自动新建新收文下级办件、阅件子分类结束...");
    }
    ...
    private void generatePermission(V3xOrgAccount account) throws BusinessException {
        ...
        try {
            ...
            configManager.saveInitCmpConfigData(EnumNameEnum.edoc_new_qianbao_permission_policy.name(),
                account.getId());
