...
public class ElementManagerImpl implements ElementManager {
    ...
    public FlipInfo getElementList(FlipInfo flipInfo, Map<String, String> paramMap) throws BusinessException {
        ...
        if (!ElementRoleHelper.checkByElementResourceCode(user.getId(), user.getLoginAccount(), InfoConstant.InfoRole_2_DataBase)) {
        ...
        if (Strings.isNotBlank(paramMap.get(ElementQueryCondition.condition.name()))) {
        	if(Strings.isNotBlank(paramMap.get(ElementQueryCondition.name.name()))) {
        		Integer appType = Strings.isBlank(paramMap.get("appType")) ? ElementAppTypeEnum.appType_default.getKey() : Integer.parseInt(paramMap.get("appType"));
	 ...
	 * @throws BusinessException
	 */
	public List<GovElement> getElementList(Integer appType, Long domainId, Boolean hasOpinionElement) throws BusinessException {
	...
	public List<GovElement> exportElementExcel(Map<String, String> paramMap, HttpServletResponse response) {
	     ...
	     try {
		    ...
		    /** (应用设置) 报送单元素导出Excel */
			appLogManager.insertLog(user, AppLogAction.Information_Element_Excel, user.getName(), dataRecord.getTitle());
			return elementList;
			...
			try {
				...
				for (int i = 0; i < sIds.length; i++) {
					...
					if(state == GovElement.C_iStatus_Active) {
						/** (应用设置) 报送单元素启用 */
						appLogManager.insertLog(user, AppLogAction.Information_Element_Enable, user.getName(),element.getName());
					} else {
						/** (应用设置) 报送单元素停用 */
						appLogManager.insertLog(user, AppLogAction.Information_Element_Disabled, user.getName(),element.getName());
					}
				}
