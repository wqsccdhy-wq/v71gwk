    ...
    public Boolean hasReportAuth(String appCategoryId, User user) {
        ...
        try {
            if (user.getLoginAccount().toString().equals(appCategoryId)) {
                if (user.isAdmin()) {
                    hasAuth = true;
                } else if (orgManager.isRole(user.getId(), user.getLoginAccount(),
                    OrgConstants.Role_NAME.MeetingRoomAdmin.name())) {
                    hasAuth = true;
                } else {
                    logger.info(String.format("拒绝访问，当前用户%s即不是单位/集团/系统管理员，又不是会议室管理员，无法制作报表!", user.getId().toString()));
                }
            } else {
                logger.info(String.format("拒绝访问，当前用户%s所在单位%S与请求归属单位%S不一致!", user.getId().toString(), appCategoryId,
                    user.getLoginAccount().toString()));
            }
        } catch (Exception e) {
            logger.error("", e);
        }
