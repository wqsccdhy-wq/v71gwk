	...
	public void importOrg(String resourceId, FlipInfo flipInfo) {
		...
		for (Short grade : gradeList) {
			...
			for (long i = 0; i < count; i++) {
				List<OrgUnitTemp> lists = orgUnitTempManager.findOrgUnitTempByGrade(grade, isFlag, resourceId, flipInfo);
	...
	public void importEntry(OrgUnitTemp orgUnitTemp, String resourceId) {
		...
		try {
			if (newAccount != null && admin != null) {
				...
				String info = "生成单位结果,单位:" + name + "|id:" + id + "|result:" + result + "|messageStatus:" + messageStatus + "|耗时:" + time + "秒";
				LOGGER.info(info);
				System.out.println(info);
			}
		} catch (Exception e) {
